...
public class HuffmanEncoding {
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 0; i < sentence.length(); i++) { //go through the sentence
            array[(int) sentence.charAt(i)]++; //increment 
        }
  ...
  //FILL THIS IN:
                ...
                //set the frequency of that Tree
            	J.frequency = (array[i]);
        ...
        while (PQ.size() > 1) { //while there are two or more Trees left in the forest
        	...
        	k.smallestLetter = a.root.smallestLetter;
        
        	k.leftChild = left;
        	k.rightChild = right;
        	//this code should be within TOPIC3 of the notes
        ...
        //you call the getCode() method on the Huffman Tree object for each character in the sentence.

for(int j =0; j <= sentence.length();j++) {
        	System.out.println(HuffmanTree.getCode(sentence.charAt(j)));
